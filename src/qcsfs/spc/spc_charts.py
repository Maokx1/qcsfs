...
def check_spc_rules(
    ...
    sigma_lines: list[int] | list[float],
    is_xbar: bool = True,
) -> list[list[int]]:
    """
    ...
    marked_values = [
        [idx for idx, x in enumerate(data) if x > sigma_lines[0] or x < sigma_lines[-1]]
    ]
    if is_xbar:
        ...
        for idx in range(len(data) - 2):
            if (
                len(
                    pts := [
                        idx + i
                        for i, p in enumerate(data[idx : idx + 3])
                        if p > sigma_lines[1]
                    ]
                )
                >= 2
        ...
        for idx in range(len(data) - 4):
            if (
                len(
                    pts := [
                        idx + i
                        for i, p in enumerate(data[idx : idx + 5])
                        if p > sigma_lines[2]
                    ]
                )
                >= 4
        ...
        for idx in range(len(data) - 9):
            if (
                len(
                    pts := [
                        idx + i
                        for i, p in enumerate(data[idx : idx + 10])
                        if p > sigma_lines[3]
                    ]
                )
                >= 9
...
def create_xbar_chart(
    ...
    subgroups = [
        data[n : n + subgroup_size] for n in range(0, num_of_screws, subgroup_size)
    ]
    means_of_subgroups = [statistics.mean(subgroup) for subgroup in subgroups]
    xbar = statistics.mean(means_of_subgroups)
    standard_deviations_of_subgroups = [
        statistics.stdev(subgroup) for subgroup in subgroups
    ]
    ...
    s_lines = [xbar_ucl, xbar_2op, xbar_1op, xbar, xbar_1om, xbar_2om, xbar_lcl]
    ...
    plt.plot(means_of_subgroups, color="b", linestyle="-", marker="o", linewidth=3)
    # Marking points violating SPC rules
    plt.plot(
        means_of_subgroups,
        color="c",
        markevery=marked_values[3],
        linestyle="None",
        marker="o",
        label="Rule #4",
    )
...
def create_xbar_s_chart(
    ...
    (
        ...
        data, num_of_subgroups=num_of_subgroups, show_parameters=False
    )
    ...
    marked_values = check_spc_rules(
        standard_deviations_of_subgroups, [s_ucl, s_lcl], False
    )
    ...
    if show_parameters:
        print(f"Centerline (mean of means): {xbar:.4f}")
        print(
            f"Standard deviation UCL: {s_ucl:.4f}\n"
