...
def main():
    ...
    if is_data and is_model:
        ...
        try:
            labels = config_values["TRAINING"]["LABELS"].replace(" ", "").split(",")
        ...
        while True:
            ...
            if event == "Display image/Next image":
                ...
                data = nn_training.get_data(
                    _curr[0], labels, _curr[1], img_shape
                )  # type: ignore
                label = "Good" if data[0][1] == 0 else "Damaged"
                gt, predicted = nn_predicting.predict(
                    model,
                    np.asarray([data[0][0]]),
                    label,  # type: ignore
                    labels,
            ...
            if event == "Measure":
                ...
                if METHOD_STATUS == "Axis method":
                    ...
                    window["-IMAGE2-"].update(data=bio.getvalue(), size=(512, 512))
            ...
            if event == "Show all measurements":
                ...
                if METHOD_STATUS == "Axis method":
                    try:
                        df = pd.read_csv("data/measuring_results/measurements_axis.csv")

                        spc_charts.create_xbar_s_chart(
                            list(df["length"]), num_of_subgroups=10
                        )
                    except ValueError or pd.errors.EmptyDataError:
                        traceback.print_exc()
                        exit(
                            "Check that the csv files in "
                            "data/measuring_results are not empty."
                        )
                else:
                    try:
                        df = pd.read_csv("data/measuring_results/measurements_rect.csv")
                        spc_charts.create_xbar_s_chart(
                            list(df["length"]), num_of_subgroups=10
                        )
                    except ValueError or pd.errors.EmptyDataError:
                ...
                table = sg.Window(
                    layout=table_layout,
                    title=f"All good screws({METHOD_STATUS})",
                    size=(350, 260),
                    font=("Arial", 12),
                    finalize=True,
                )
