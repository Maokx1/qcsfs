...
def get_data(data_path: str | os.PathLike, label_list: list[str], label_: str,
             shape_: tuple[int, int], path_list: list = [],
             is_test: bool = False) -> list[list[np.ndarray]] | list[list[int]]:
    """
...
def create_model():
    ...
    try:
        ...
        base_learning_rate = float(
            config_values['TRAINING']['BASE_LEARNING_RATE'])
    ...
    # creating datasets
    for img, label in get_data(data_path='data/screws/train/good', label_list=labels,
                               label_=labels[0], shape_=img_shape, is_test=False, path_list=[]) + \
        get_data(data_path='data/screws/train/damaged', label_list=labels,
                 label_=labels[1], shape_=img_shape, is_test=False, path_list=[]):
        x_train.append(img)
        y_train.append(label)

    for img, label in get_data(data_path='data/screws/validation/good', label_list=labels,
                               label_=labels[0], shape_=img_shape, is_test=False, path_list=[]) + \
        get_data(data_path='data/screws/validation/damaged', label_list=labels,
                 label_=labels[1], shape_=img_shape, is_test=False, path_list=[]):
        x_val.append(img)
        y_val.append(label)

    for img, label in get_data(data_path='data/screws/test/good', label_list=labels,
                               label_=labels[0], shape_=img_shape, is_test=True, path_list=path) + \
        get_data(data_path='data/screws/test/damaged', label_list=labels,
                 label_=labels[1], shape_=img_shape, is_test=True, path_list=path):
        x_test.append(img)
        y_test.append(label)
    ...
    logging.info(f'Init learning rate = {base_learning_rate},'
    ...
    base_model = tf.keras.applications.MobileNetV2(input_shape=(img_shape[0], img_shape[1], 3), include_top=False,
                                                   weights='imagenet')
    ...
    with open(csv_path, 'w', encoding='UTF8') as f:
        writer = csv.writer(f)
        writer.writerow(['Training_accuracy', 'Validation_accuracy',
                        'Training_loss', 'Validation_loss'])
        for t_a, v_a, t_l, v_l in zip(history.history['accuracy'], history.history['val_accuracy'],
    ...
    for img_name, membership_val in zip(path, predictions):
        if membership_val < membership_boundary:
            print(f'Neural network thinks {img_name} shows good screw. '
                  f'Membership value: {round(float(membership_val), 4)}\n')
        else:
            print(f'Neural network thinks {img_name} shows damaged screw. '
                  f'Membership value: {round(float(membership_val), 4)}\n')

    predictions[predictions < membership_boundary] = 0
    predictions[predictions >= membership_boundary] = 1

    print(classification_report(y_test, predictions.astype(np.uint8),
